...
public class EntityArrow extends Entity implements IProjectile
{
    ...
    public long CREATE_ENTITY_ARROW_TIME;
    public static long DE_SPAWN_ENTITY_ARROW_TIME;
    public static boolean ENABLE_ENTITY_ARROW_REMOVE, SEND_LOG_MESSAGE;

    static {
        ((ServerCommandManager) MinecraftServer.getServer().getCommandManager()).registerCommand(new CommandBase() {
            @Override
            ...
            public String getCommandUsage(ICommandSender commandSender) {
                return ChatColor.translateAlternateColorCodes('&', "&9/arrowremove help&7: View command capabilities.");
            }

            @Override
            public void processCommand(ICommandSender commandSender, String[] args) {
                if(args.length == 0 || args.length >= 1 && args[0].equalsIgnoreCase("help")) {
                    commandSender.sendChatToPlayer(ChatMessageComponent.createFromText(ChatColor.translateAlternateColorCodes('&',
                            "&9/arrowremove reload&7: Reload configuration settings.")));
                    return;
                }
            ...
            public boolean canCommandSenderUseCommand(ICommandSender commandSender) {
                if(commandSender instanceof EntityPlayer && MinecraftServer.getServer().getConfigurationManager()
                        .isPlayerOpped(commandSender.getCommandSenderName()))
                    return true;
                else if(commandSender instanceof MinecraftServer)
                    return true;
                return false;
            }
        });
    ...
    public static void initialize() {
        InputStream is = EntityArrow.class.getClassLoader().getResourceAsStream("EntityArrowRemove.yml");
        if(is == null) {
            DE_SPAWN_ENTITY_ARROW_TIME = 1000l * 5l;

            ENABLE_ENTITY_ARROW_REMOVE = true;
            SEND_LOG_MESSAGE = true;
        } else {
            FMLLog.info("[EntityArrow] Loading configuration.");
            FileConfiguration configuration = YamlConfiguration.loadConfiguration(is);
            DE_SPAWN_ENTITY_ARROW_TIME = 1000l * configuration.getInt("getSettings.getTime", 10);

            ENABLE_ENTITY_ARROW_REMOVE = configuration.getBoolean("getSettings.isEnable", true);
            SEND_LOG_MESSAGE = configuration.getBoolean("getSettings.isSendMessage", false);
        }

        FMLLog.info("Time to remove entity arrow: %s, sending message to console: %s, enable: %s.",
                DE_SPAWN_ENTITY_ARROW_TIME / 1000l, SEND_LOG_MESSAGE, ENABLE_ENTITY_ARROW_REMOVE);
    }
    ...
    public EntityArrow(World par1World)
    {
        ...
        if(ENABLE_ENTITY_ARROW_REMOVE)
            CREATE_ENTITY_ARROW_TIME = System.currentTimeMillis() + DE_SPAWN_ENTITY_ARROW_TIME;
    }
    ...
    public void onUpdate()
    {
        ...
        {
            if(ENABLE_ENTITY_ARROW_REMOVE && CREATE_ENTITY_ARROW_TIME - System.currentTimeMillis() <= 0) {
                if(SEND_LOG_MESSAGE)
                    FMLLog.info("EntityArrow in world(%s) at pos (x: %s, y: %s, z: %s) has been removed!",
                            worldObj.getWorldInfo().getWorldName(), posX, posY, posZ);
                setDead();
                return;
            }
